...
class SwiftypeAppSearchClient {
  ...
  indexDocuments(engineName, documents) {
    documents = documents.map((d) => this._validateAndNormalizeDocument(d))
  ...
  _validateAndNormalizeDocument(document) {
    const documentKeys = Object.keys(document)
    const missingKeys = REQUIRED_TOP_LEVEL_KEYS.filter((key) => !documentKeys.includes(key))
